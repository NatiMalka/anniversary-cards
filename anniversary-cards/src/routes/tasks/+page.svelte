<script>
  ...
  import { tasks, completions, dailyTasks, specialTasks, secretTasks } from '$lib/stores/tasks.js';
  ...
  function claim(task) {
    const result = tasks.complete(task, $user.id);
    ...
    if (result.rewardType === 'free_pack') {
      ...
      // In Phase B: grant free pack to user's account
    } else {
          ...
          {#each sec.list as task}
            ...
            <div class="task-card surface" class:done style="--accent:{sec.accent}">
              {#if task.type === 'secret' && !done}
                <!-- secret: blurred until completed -->
