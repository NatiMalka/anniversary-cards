<script>
	import { collection, collectedCount, collectionYears } from '$lib/stores/collection.js';
	...
	function closeModal(e) {
		if (e.target === e.currentTarget) modalCard = null;
	}
	function clearCollection() {
		if (!confirm('למחוק את כל הקלפים באלבום? (לצורכי בדיקה בלבד)')) return;
		collection.reset();
	}
...
<main>
	...
	<header class="album-head">
		...
		<button class="btn btn-ghost btn-sm clear-btn" on:click={clearCollection}
				title="נקה אלבום (POC בלבד)">
			🗑 נקה
		</button>
	...
	<div class="filters">
		<!-- year tabs -->
		<div class="tab-group">
			<button class="tab" class:active={activeYear === 'all'} on:click={() => (activeYear = 'all')}>
				כל השנים
			</button>
		...
		<div class="tier-group">
			{#each tiers as t}
				<button
					...
					style={activeTier === t && t !== 'all'
						? `background:${tierColors[t]}; border-color:transparent;`
						: ''}
...
<style>
	...
	.progress-bar {
		height: 100%;
		background: linear-gradient(90deg, #c89a1a, #f5c451);
		border-radius: 999px;
	...
	.slot--filled:hover {
		...
		filter: drop-shadow(0 8px 20px rgba(245, 196, 81, 0.3));
		z-index: 10;
	}
	.slot-card {
		width: 100%;
	...
	/* keep slots pointer-free; interaction only on click (modal) */
	.slot--filled :global(.card) {
		pointer-events: none !important;
	}
	/* hide holographic shine/glare on album thumbnails — effects only in the modal */
	.slot-card :global(.card__shine),
	.slot-card :global(.card__glare) {
		display: none !important;
	}
	/* also block the non-interactive hover rule from base.css */
	.slot-card :global(.card:not(.interactive):hover) {
	...
	.modal-close {
		position: absolute;
		top: -0.5rem;
		left: -0.5rem; /* RTL: close button on left */
		z-index: 10;
		width: 2rem;
		height: 2rem;
		border-radius: 50%;
		border: 1px solid rgba(255, 255, 255, 0.2);
		background: rgba(30, 14, 50, 0.9);
		color: var(--ink-dim);
		font-size: 0.85rem;
		cursor: pointer;
		display: grid;
